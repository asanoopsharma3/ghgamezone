...
const STATUS_MAP = {
  ...
  112: { success: false, message: "Subscription in progress. Please wait a moment.", type: "info" },
  ...
  644: { success: false, message: "Duplicate subscription. Please try again later.", type: "error" },
...
const extractCode = (value) =>
  value.trim().replace(/^unknown status(?: code)?:?\s*/i, "").toLowerCase();
...
export const resolveCgwCallbackNotice = (params) => {
  ...
  if (fromReasonCode && !fromReasonCode.success) return fromReasonCode;

  if (fromStatus && STATUS_MAP[extractCode(status)]) {
    if (fromStatus.success && isFlaggedSuccess) return fromStatus;
    if (!fromStatus.success) return fromStatus;
  }
